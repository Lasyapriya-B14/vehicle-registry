...
function DLUploader({ initialData, owner_id }: AadharFormProps) {

    const [driving_license_url, setDLUrl] = useState<string | null>(initialData.driving_license_url);
    ...
    const onDrop = useCallback(async (acceptedFiles: File[]) => {
        ...
        setDLUrl(data.publicUrl);

        try {
            const res = await fetch("/api/setOwnerDL", {
                method: "POST",
    ...
    return (
        <div className='flex h-full bg-white dark:bg-black dark:border-neutral-300 p-5 flex-col gap-5 rounded-lg border-2 border-neutral-100 '>
            <p className='text-base font-medium'>Driving License Document Attachment</p>
            {
                !driving_license_url &&
            ...
            {driving_license_url ? (
                <Link href={driving_license_url} target='_blank' className='italic flex gap-2'>
